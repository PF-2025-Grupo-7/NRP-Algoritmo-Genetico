...
def mutate_reassign_shift(sol, problema, max_attempts=20):
    ...
    for p in range(problema.num_profesionales):
        if p == p_origen:
            continue
        if problema.info_profesionales[p]['skill'] != problema.info_profesionales[p_origen]['skill']:
            continue
        ...
        if (prev, t) in problema.secuencias_prohibidas:
            continue
        if (t, nxt) in problema.secuencias_prohibidas:
            continue
        if int((matriz[p] > 0).sum()) >= int(problema.info_profesionales[p]['t_max']):
            continue
        candidatos.append(p)
        attempts += 1
...
def mutate_swap_same_day(sol, problema):
    ...
    def viola_seq(p, dia, nuevo_turno):
        ...
        if (nuevo_turno, nxt) in problema.secuencias_prohibidas:
            return True
        return False
    if viola_seq(p1, d, t2) or viola_seq(p2, d, t1):
        return sol
    def count_after_swap(p, dia, turno_new):
        cnt = int((matriz[p] > 0).sum())
        if matriz[p, dia] > 0 and turno_new == 0:
            cnt -= 1
        if matriz[p, dia] == 0 and turno_new > 0:
            cnt += 1
        return cnt
    if count_after_swap(p1, d, t2) > int(problema.info_profesionales[p1]['t_max']):
        return sol
    if count_after_swap(p2, d, t1) > int(problema.info_profesionales[p2]['t_max']):
        return sol
